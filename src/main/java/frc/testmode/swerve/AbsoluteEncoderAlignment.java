...
public class AbsoluteEncoderAlignment implements Subsystem, Sendable {
    public static Command StartAlignmentCommand(RobotConfiguration robotConfiguration, RobotFactory robotFactory) {
        
        var alignment = new AbsoluteEncoderAlignment(robotConfiguration, robotFactory);
    ...
    private AbsoluteEncoderAlignment(RobotConfiguration robotConfiguration, RobotFactory robotFactory) {
        this.config = robotConfiguration;
        this.factory = robotFactory;
    ...
    void disableDriveController() {
        disabledSubsystem = (Subsystem)factory.getInstance(frc.robot.subsystems.DriveSubsystem.class);
        if (disabledSubsystem != null) { CommandScheduler.getInstance().unregisterSubsystem(disabledSubsystem);}

        disabledCommand = (Command)factory.getInstance(frc.robot.commands.DriveCommands.class);
        if (disabledCommand != null) { disabledCommand.cancel(); }
    }
    void enableSubsystem() {
        if (disabledSubsystem != null) { CommandScheduler.getInstance().registerSubsystem(disabledSubsystem); }
    }
    ...
    class ModuleAlignment implements Sendable {
        ...
        SparkMax getDriving() { return module.DrivingMotor().CANSparkMax();}
        SparkMax getTurning() { return module.TurningMotor().CANSparkMax();}
        double getAngle() { return module.Encoder().getPosition().magnitude(); }
        ...
        public void initSendable(SendableBuilder builder) {
            ...
            builder.addIntegerProperty("Align " + module.Name() + " Faults", ()-> getDriving().getFaults().rawBits, null);
